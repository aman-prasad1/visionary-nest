...
interface Step8Props {
  items: { title: string, org: string, dateRange: string, bullets: string }[];
...
const Step8: React.FC<Step8Props> = ({ items, handleItemChange, handleAddItem, handleRemoveItem, errors }) => {
  return (
    <div className="w-full max-w-lg mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-center text-white">Education</h2>
      {errors.education && <p className="text-red-500 text-xs mb-2">{errors.education}</p>}
          ...
          <div key={index} className="bg-white/10 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <input
                  type="text"
                  placeholder="Title (e.g., B.Tech in CSE)"
                  value={item.title}
                  onChange={(e) => handleItemChange(index, 'title', e.target.value)}
                  className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                {errors[`education_title_${index}`] && <p className="text-red-500 text-xs mt-1">{errors[`education_title_${index}`]}</p>}
              </div>
              <div>
                <input
                  type="text"
                  ...
                  className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                {errors[`education_org_${index}`] && <p className="text-red-500 text-xs mt-1">{errors[`education_org_${index}`]}</p>}
              </div>
              <input
                type="text"
                placeholder="Date Range (e.g., 2021 - 2025)"
